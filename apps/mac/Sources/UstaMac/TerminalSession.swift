...
final class TerminalSession: ObservableObject, Identifiable {
    ...
    private var outboundContinuation: AsyncStream<Usta_V1_PtyClientMsg>.Continuation?
    ...
    func start(stub: Usta_V1_Usta.Client<HTTP2ClientTransport.Posix>) {
        let id = self.id
        let (stream, cont) = AsyncStream<Usta_V1_PtyClientMsg>.makeStream(bufferingPolicy: .bufferingNewest(64))
        self.outboundContinuation = cont

        var attachMsg = Usta_V1_PtyClientMsg()
        attachMsg.attach = .with { $0.terminalID = id }
        cont.yield(attachMsg)

        var resizeMsg = Usta_V1_PtyClientMsg()
    ...
    private func handleServer(_ msg: Usta_V1_PtyServerMsg) {
        switch msg.kind {
        case .output(let o):
