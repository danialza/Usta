...
final class WorkspaceBus: ObservableObject {
    @Published var events: [Usta_V1_Event] = []
    @Published var roles: [Usta_V1_Role] = []
    ...
    func orderedRoles(_ all: [Usta_V1_Role]) -> [Usta_V1_Role] {
    ...
    func bottleneck() -> (name: String, dependents: Int, cycle: Bool)? {
        ...
        func effectivelyReady(_ r: Usta_V1_Role) -> Bool {
        ...
        func plainScore(_ r: Usta_V1_Role) -> Int {
    ...
    func start(workspaceID: String, client: UstaClientModel, roles: [Usta_V1_Role]) {
        self.client = client
        self.roles = roles
        pollTask?.cancel()
    ...
    func updateRoles(_ rs: [Usta_V1_Role]) { self.roles = rs }
    ...
    private func refresh(workspaceID: String) async {
           ...
           roles.contains(where: { $0.name == current && state(of: current) == .pending }) {
            ...
            if next != pinnedBottleneckName {
                ...
                if let n = next, shouldAutoRegen(n) {
                    NotificationCenter.default.post(name: .ustaAutoRegenerate, object: n)
        ...
        for name in newlyReady where shouldAutoRegen(name) {
            NotificationCenter.default.post(name: .ustaAutoRegenerate, object: name)
