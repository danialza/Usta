...
//! gRPC Unix socket.
//!
//! Context from env (set by Atelier when it launches a CLI pane):
//!   ATELIER_SOCKET        UDS path of atelierd
...
use atelier_proto::v1::{
    atelier_client::AtelierClient, ListEventsRequest, PublishEventRequest,
};
...
async fn connect() -> anyhow::Result<AtelierClient<Channel>> {
