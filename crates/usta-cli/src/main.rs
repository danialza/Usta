...
use usta_core::default_socket_path;
use usta_proto::v1::{
    usta_client::UstaClient, pty_client_msg::Kind as PtyCKind,
...
#[derive(Parser, Debug)]
#[command(name = "ustacli", version, about = "Usta CLI client")]
struct Args {
...
fn resolve_ws(arg: String) -> Result<String> {
    ...
    CliState::load()
        .active_workspace_id
        .ok_or_else(|| anyhow::anyhow!("no active workspace — run `ustacli use <path>` or pass --workspace-id"))
}
...
async fn connect(socket: PathBuf) -> Result<UstaClient<tonic::transport::Channel>> {
    let channel = Endpoint::try_from("http://[::]:50051")?
        ...
        .await
        .context("connect to ustad UDS")?;
    Ok(UstaClient::new(channel))
}

fn find_daemon_bin() -> PathBuf {
    // Sibling next to ustacli (same target dir).
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let p = dir.join("ustad");
...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Ping => {
            let r = client.ping(PingRequest { client_name: "ustacli".into() }).await?.into_inner();
        ...
        Cmd::Term { sub } => match sub {
            TermCmd::New { workspace_id, shell } => {
                let ws_id = if workspace_id.is_empty() {
                    CliState::load()
                        .active_workspace_id
                        .ok_or_else(|| anyhow::anyhow!("no active workspace — run `ustacli use <path>` first or pass --workspace-id"))?
                } else { workspace_id };
        ...
        Cmd::Team { sub } => match sub {
            ...
            TeamCmd::List { workspace_id } => {
                ...
                if shown == 0 {
                    println!("{}", "(no workspace roles yet — run `ustacli team apply`)".dimmed());
                }
            }
...
fn show_active() -> Result<()> {
    ...
    match (&st.active_workspace_path, &st.active_workspace_id) {
        ...
        _ => println!("{}", "(no active workspace — `ustacli use <path>`)".dimmed()),
    }
    Ok(())
}

async fn doctor(client: &mut UstaClient<tonic::transport::Channel>, sock: &std::path::Path) -> Result<()> {
    println!("{} {}", "usta doctor".bold(), env!("CARGO_PKG_VERSION").dimmed());
    println!();
...
async fn attach(client: &mut UstaClient<tonic::transport::Channel>, id: String) -> Result<()> {
